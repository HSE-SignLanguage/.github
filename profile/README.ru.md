...
Полноценный отчёт по бенчмаркам (точность и задержка на CPU и GPU) сейчас готовится — см. [Roadmap](#-roadmap-и-открытые-исследовательские-вопросы).
...
Backend на Go принимает кадры с веб-камеры по WebSocket (батчами по 32) или извлекает кадры из загруженного видео с помощью FFmpeg, пересылает каждый батч в ML-сервис и опционально «причёсывает» сырой дословный текст в естественный, грамматически связный текст через LLM, сохраняя контекст между батчами для связности.
...
Все три сервиса контейнеризованы и деплоятся независимо через [Dokploy](https://dokploy.com/) — self-hosted PaaS с логами и откатом по каждому сервису. Хакатон-окружение сейчас недоступно публично — напишите нам, если хотите посмотреть демо.
...
Сделано студентами НИУ ВШЭ за 48 часов хакатона · декабрь 2025
