...
- **Отсечение неуверенных ответов:** ML-сервис отклоняет `no gesture`, низкую уверенность и малый отрыв top-1 от top-2 вместо возврата любого argmax. Live-распознавание требует два совпадающих принятых предсказания и разрешает повтор того же жеста только после нейтральных или отклонённых окон.
...
- Словарь закрыт и содержит около 1600 меток из [Slovo](https://github.com/hukenovs/slovo); имена, новые слова и региональные варианты могут отсутствовать в распределении.
...
- Публичных бенчмарков точности и задержки пока нет. Live-инстанс имеет намеренно небольшие лимиты и при занятости может вернуть `503`.
